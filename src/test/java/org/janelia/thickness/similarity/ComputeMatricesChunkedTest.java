...
public class ComputeMatricesChunkedTest implements Serializable
{
	...
	public void test() throws InterruptedException, ExecutionException
	{
		final SparkConf conf = new SparkConf()
				...
				// Only use one CPU. Spark overhead kills test with heap growing
				// bigger than 512MB if using all CPUs.
				.setMaster( "local[1]" )
		...
		final int range = 15;
		final ImagePlus firstImp = new ImagePlus( String.format( pattern, start ) );
		final int w = firstImp.getWidth();
		final int h = firstImp.getHeight();
		final int[] dim = { w / ( 1 << scaleLevel ), h / ( 1 << scaleLevel ) };
		final int[] stride = new int[] { dim[ 0 ] / 2, dim[ 1 ] / 2 };
		final int[] radius = { stride[ 0 ] / 2, stride[ 0 ] / 2 };
		final ArrayList< Integer > arange = Utility.arange( start, stop );
		final JavaPairRDD< Integer, FloatProcessor > files = sc.parallelize( arange )
				.mapToPair( new Utility.LoadFileFromPattern( pattern ) )
				.mapToPair( t -> Utility.tuple2( t._1().intValue() - start, t._2() ) )
		...
		final ComputeMatricesChunked test = new ComputeMatricesChunked( sc, files, stepSize, range, dim, true );

		final Map< Tuple2< Integer, Integer >, FloatProcessor > strips =
				test.run( new DefaultMatrixGenerator.Factory( dim ), range, stride, radius ).collectAsMap();
		sc.close();
		...
		for ( int y = radius[ 1 ], yIndex = 0; y < dim[ 1 ]; y += stride[ 0 ], ++yIndex )
		{
			for ( int x = radius[ 0 ], xIndex = 0; x < dim[ 0 ]; x += stride[ 0 ], ++xIndex )
			{
				...
				final RandomAccessibleInterval< FloatType > stripRef = MatrixStripConversion.matrixToStrip( ImageJFunctions.wrapFloat( new ImagePlus( "", matrixFp ) ), range, new FloatType() );
				final Img< FloatType > stripComp = ImageJFunctions.wrapFloat( new ImagePlus( "", strip ) );
				...
				{
					final float ref = refC.next().get();
					final float comp = compC.next().get();
